...
# --- Anti-Bot Cloudflare Bypass Session ---
    ...
    if os.path.exists(cookie_file):
        with open(cookie_file, 'r') as f:
            for line in f:
                ...
                if len(parts) >= 7:
                    scraper.cookies.set(parts[5], parts[6], domain=parts[0])
    return scraper
...
def get_episode_metadata_and_m3u8(episode_url, session):
    ...
    if m3u8_match:
        ...
        raise Exception("M3U8 URL not found! 403 Blocked or Invalid Link.")
...
async def execute_downloads(update: Update, context: ContextTypes.DEFAULT_TYPE, chat_id, target_urls):
    ...
    for i, url in enumerate(target_urls, 1):
        ...
        try:
            ...
            if thumb_url:
                try:
                    img_data = cloudscraper.create_scraper().get(thumb_url, timeout=15).content
                    if not os.path.exists('downloads'):
                        os.makedirs('downloads')
